...
class Model(modules.Module):
    ...
    def on_step_end(self, evaluate, save, log_figure, callback_path, epoch, step, inputs, targets, dataset_eval, eval_steps, verbose_eval, writer, recompute_metrics, keep_last_k, tag="step"):
        ...
        # Log Figure - only when evaluating
        if log_figure and callback_path and evaluate:
            self.eval()
    ...
    def fit(
        ...
        # Init Profiler
        profiler = None
        if enable_profiling and callback_path is not None:
            print("Enable Profiling")
            profiler_log_dir = os.path.join(callback_path, "profiler_logs")
            ...
            print(f"Profiler initialized. Logs will be saved to: {profiler_log_dir}")
            print("To view profiling results, run: tensorboard --logdir=" + profiler_log_dir)
        ...
        # Exception Handler
            ...
            if profiler is not None:
                profiler.stop()
                print("Profiler stopped due to exception")
        ...
        # Profiler Cleanup
            ...
            if profiler is not None:
                ...
                print(profiler.key_averages().table(sort_by="cuda_memory_usage", row_limit=20))
                
                if callback_path is not None:
                    trace_file = os.path.join(callback_path, "trace.json")
