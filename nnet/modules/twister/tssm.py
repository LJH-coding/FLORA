...
class TSSM(nn.Module):
    ...
    def __init__(
        ...
        self.use_semantic_latent = use_semantic_latent
        self.state_stoch_size = self.stoch_size * (2 if self.use_semantic_latent else 1)

        # [stoch_t, action_t] -> transition features
        self.action_mixer = modules.MultiLayerPerceptron(
            dim_input=self.state_stoch_size * self.discrete + self.num_actions,
        ...
        # Dynamics predictor dt -> latent logits (z only / z+d concatenated)
        self.latent_predictor = modules.Linear(
            in_features=self.hidden_size, 
            out_features=self.state_stoch_size * self.discrete,
    ...
    def get_dist(self, dist_params):
        return torch.distributions.Independent(
            distributions.OneHotDist(logits=dist_params['logits'], uniform_mix=self.uniform_mix), 
            1
        )

    def get_stoch(self, deter):
        latent_logits = self.latent_predictor(deter)
        logits = latent_logits.reshape(deter.shape[:-1] + (self.state_stoch_size, self.discrete))
        dist_params = {"logits": logits}
        stoch = self.get_dist(dist_params).mode().flatten(start_dim=-2, end_dim=-1)
        return stoch
    ...
    def initial(self, batch_size=1, seq_length=1, dtype=torch.float32, device="cpu", detach_learned=False):
        ...
        initial_state = structs.AttrDict(
            logits=torch.zeros(batch_size, seq_length, self.state_stoch_size, self.discrete, dtype=dtype, device=device),
            stoch=torch.zeros(batch_size, seq_length, self.state_stoch_size * self.discrete, dtype=dtype, device=device),
    ...
    def get_feat(self, state, blocks_deter_id=None):
        return torch.cat([state["stoch"], state["deter"] if blocks_deter_id is None else state["blocks_deter"][blocks_deter_id]], dim=-1)
    ...
    def forward_img(self, prev_states, prev_actions, mask, return_att_w=False, return_blocks_deter=False, imagine=False):
        ...
        # Joint linear logits for latent state
        latent_logits = self.latent_predictor(deter)
        logits = latent_logits.reshape(deter.shape[:-1] + (self.state_stoch_size, self.discrete))
        dist_params = {"logits": logits}
        stoch = self.get_dist(dist_params).rsample().flatten(start_dim=-2, end_dim=-1)

        if imagine and self.diffusion_model is not None:
            condition = torch.cat([deter, stoch], dim=-1)
            stoch = self.diffusion_model.sample(condition)
            logits = stoch.reshape(stoch.shape[:-1] + (self.state_stoch_size, self.discrete))
