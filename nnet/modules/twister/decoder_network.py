...
from nnet.modules.twister.convnet import ResidualBlock, Upsample, RMSNorm2d
...
class DecoderNetwork(nn.Module):
    ...
    def forward_cnn(self, x, return_first=False):
        ...
        if return_first:
            first_feat = self.cnn.layers[0](x)
            x = first_feat
            for layer in self.cnn.layers[1:]:
                x = layer(x)
        else:
            first_feat = None
            x = self.cnn(x)
        ...
        return (obs_dist, first_feat) if return_first else obs_dist
    
    def forward(self, inputs, return_first=False):

        # Outputs
        outputs = self.forward_cnn(inputs, return_first=return_first)
