...
class InnerModel(nn.Module):
    def __init__(self, cfg: InnerModelConfig) -> None:
        ...
        self.t_emb = FourierFeatures(cfg.dim)
        self.r_emb = FourierFeatures(cfg.dim)
        self.cond_emb = nn.Linear(cfg.cond_dim, cfg.dim)
        ...
        self.unet = UNet(cfg.dim, cfg.depths, cfg.channels, cfg.attn_depths)
        self.norm_out = GroupNorm(cfg.channels[0])
        self.conv_out = Conv3x3(cfg.channels[0], cfg.img_channels)
        nn.init.zeros_(self.conv_out.weight)

    def forward(self, x: Tensor, t: Tensor, r: Tensor, c: Tensor) -> Tensor:
        cond = self.cond_proj(self.t_emb(t) + self.r_emb(r) + self.cond_emb(c))
        x = self.conv_in(x)
