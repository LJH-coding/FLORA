...
class Normalizer:
    ...
    def __init__(self, mode='minmax', mean=None, std=None):
        assert mode in ['minmax', 'mean_std', 'identity'], "mode must be 'minmax' or 'mean_std' or 'identity'"
        self.mode = mode
...
class MeanFlow:
    def __init__(
        ...
        channels=1,
        image_size=32,
        cond_dim=None,  # Feature vector dimension (None for unconditional)
        ...
        self.image_size = image_size  # Store as (H, W)
    ...
    def loss(self, model, x, c=None):
        ...
        if c is not None:
            ...
            c = torch.where(cfg_mask.view(-1, 1), uncond, c)
        ...
        # forward pass
        # u = model(z, t, r, y=c)
        model_partial = partial(model, c=c)
        jvp_args = (
    ...
    @torch.no_grad()
    def sample(self, model, cond=None, sample_steps=1, device='cuda'):
        model.eval()

        if cond is None:
            assert True, "Conditional input is required for sampling"
